...
public class PersonDetailsService implements UserDetailsService {  // "implements" ties ModelRepo to Spring Security
    ...
    public  List<Person>list(String name, String username) {
        return personJpaRepository.findByNameContainingIgnoreCaseOrUsernameContainingIgnoreCase(name, username);
    }

    // custom query to find anything containing term in name or username ignoring case
    public  List<Person>listLike(String term) {
        return personJpaRepository.findByNameContainingIgnoreCaseOrUsernameContainingIgnoreCase(term, term);
    }
